...
featureNames <- suppressWarnings(
  unique(c(names(tsfeatures(M3[[1907]]$x, features = features)),
...
server <- function(input, output) {
  evolved.ts <- eventReactive(input$do, {
    selected.features <- unlist(lapply(featureNames, function(x){ifelse(input[[x]] != '0', x, NA)})) %>% na.omit()
    n.features <- length(selected.features)
    target <- rep(NA, n.features)
    for(i in 1:n.features){target[i] = as.numeric(input[[selected.features[i]]])}
    seasonal <- 
    ...
    withProgress(message = 'Generating data', detail = "0%", {
      ...
      while(ifelse(is.null(dim(evolved.ts)), 0 < 1, dim(evolved.ts)[2] < input$Number)){
        GA <- myGA(type = "real-valued", fitness = fitness, features = features, seasonal = seasonal, 
    ...
    }else{msts(evolved.ts[,1:(input$Number)], seasonal.periods = freq)}})
