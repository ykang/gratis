...
#' feasts::autoplot(df, value)
#'
#' @export
generate.mar <- function(x, length=100, nseries=10, frequency=1, ...) {
  generate_gratis(x, length, nseries, frequency, ...)
}

#' @rdname generate.mar
#' @export
generate.ets <- function(x, length=100, nseries=10, frequency=1, ...) {
  generate_gratis(x, length, nseries, frequency, ...)
}

#' @rdname generate.mar
#' @export
generate.Arima <- function(x, length=100, nseries=10, frequency=1, ...) {
  generate_gratis(x, length, nseries, frequency, ...)
}

# Generic generate function for models in gratis package
generate_gratis <- function(x, length=100, nseries=10, frequency=1, ...) {
  tsmatrix <- ts(matrix(0, nrow=length, ncol=nseries), frequency=frequency)
