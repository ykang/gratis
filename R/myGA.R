...
#'
#' @examples
myGA <- function(type = c("binary", "real-valued", "permutation"), 
               fitness, ..., n,
               min, max, nBits,
               ...
               { if(is.RStudio()) gaMonitor else FALSE} 
               else FALSE,
               seed = NULL) 
{
  ...
  for(iter in seq_len(maxiter))
  {
    ...
    { Fitness0 <- foreach(i = seq_len(popSize)) %DO% {
      if (is.na(Fitness[i])) {
        fitness(Pop[i, ], ...)
      }else {list(value = Fitness[i], x = popTs[,i])}
    }
    Fitness <- sapply(Fitness0, function(x){x$value})
    popTs <- sapply(Fitness0, function(x){x$x})
    }
