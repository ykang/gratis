...
generate_ts <- function(n.ts = 1, freq = 1, nComp = NULL, n = 120) {
    ...
    while (count <= n.ts) {
      ...
      if (freq != 1) {
        ...
        if (!(any(is.na(x)) || (max(abs(x), na.rm = TRUE) > 1e5))) {
          ...
          simulated.mixture.data[[number]] <- list()
          simulated.mixture.data[[number]]$x <- x
          simulated.mixture.data[[number]]$pars <- pars
          count = count + 1
        }
      } else{
        # nonseasonal data
    ...
    return(simulated.mixture.data)
  }



#' Simulate mutiple seasonal time series from random parameter spaces of the mixture autoregressive (MAR) models.
#'
#' Simulate mutiple seasonal time series from random parameter spaces of the mixture autoregressive (MAR) models.
#' @param seasonal.periods a vector of seasonal periods of the time series to be simulated.
#' @param n length of the simulated time series.
#' @param nComp number of mixing components when simulating time series using MAR models.
...
#===========================================================================
# generated weights for the simulation of msts
#===========================================================================
